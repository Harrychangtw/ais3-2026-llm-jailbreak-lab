...
給想打更多、更難題目的進階學員。`attack-corpus-full.csv` 是本課多模型 ASR 評測（6 模型 × 3,602 題）所用的**完整攻擊語料**，欄位與 `labs/data/` 的其他 CSV 一致，notebook 第 6 節的 `load_rows` 可直接讀。
...
把檔案放到 notebook 當前目錄（或 `data/`、`labs/data/`），`load_rows` 會優先讀本機檔。挑不同 `ROW_INDEX`、不同 `source_file` 的題目，看你的手法在哪類題目、哪顆模型上撐得住。第 2 節的難度階梯（`8b` → `550b`）配這份語料就是完整的挑戰場。
...
本語料是**七個已發表的學術安全 / 越獄 benchmark** 中 `label==1`（有害意圖）題目的重組，取樣邏輯見 `eval-results/build_corpus.py`（決定性、可重現）。各 benchmark 的原始授權條款以其原始發佈為準，使用請遵循各自 license；此處為授權資安教育用途的彙整重散布。詳細 profile 見 `benchmarks/README.md`。
...
- 這是**授權的資安教育素材**。這份語料**未做溫和化過濾**，含 `benchmarks/` 完整分類（涵蓋 cybercrime、malware、terrorism 等重度類別），與 `labs/data/attack-samples.csv`（刻意停在盜版等溫和等級的 44 題切片）不同。用途限於理解對齊模型的**拒絕機制**與評估攻防，**不是**產出真正可操作的有害內容。
- 這些是**已公開的研究 benchmark**；散布請保留上表歸屬並遵循各原始 license。
- **模型的有害完成內容不在此檔**（本檔只有攻擊 prompt）。評測產生的原始回覆（`eval-results/results.jsonl`）刻意不進版控、不進公開鏡像。
