...
主 notebook：`jailbreak-redteam.ipynb`（進場 smoke test → 第一站 mirror → 第二站 N = 5 成功率 → 第 7.5 節 送排行榜（選用）→ 第三站 防禦 + 誤殺率）。
...
**競賽兩輪 + 排行榜（選用）**：課堂用兩輪串起三站，第 1 輪打 `ais3/llama-3.1-8b`（小、好打進）、第 2 輪把 notebook 第 2 節的 `TARGET_KEY` 改成 `"70b"` 打 `ais3/llama-3.3-70b`（較硬），比較同一手法在 8B vs 70B 的成功率（安全性 vs 規模）。講師會開一個即時排行榜、判分在伺服器端；但 `LEADERBOARD_URL` 留空或連不上時 notebook 照跑不誤，三站 solo 流程不依賴排行榜。
...
目標模型是一組 `TARGET_MODELS` 階梯（8B 到 550B，均帶 `ais3/` 前綴）；`TARGET_KEY` 預設 `"8b"`（第 1 輪），第 2 輪改成 `"70b"`。不帶 `ais3/` 前綴會回 403。
...
- **`data/attack-samples.csv`**：第二站的攻擊題庫切片（44 題，取自 HarmBench / StrongREJECT / SORRY-Bench 的溫和子集），全部 `label=1`，涵蓋 illegal / cybercrime / fraud_scam / 假訊息 / 騷擾 / 隱私 / 侵權七類。
- **`data/eval-mix.csv`**：第三站評估用的混合切片（40 列），有害（`label=1`，10 列）＋ 看起來危險但其實無害（`label=0`）＋ 一般無害（`label=0`），共 30 列無害，讓你同時量 recall（擋住攻擊）與誤殺率（over-refusal）。
